...
class User {
  ...
  static async getByEmail(email) {
    ...
    let users = await this.table.filter({
      email: email
    }).run();
   ...
   */
  @DBDecorators.table(tables.User)
  static async getByEmailAndPassword({
    email,
    password
  }) {
  ...
  static async updateStatus(userId, status) {
    ...
    // Validate status
    if (Object.keys(UserStatus).map(function(key) {
   ...
   */
  @DBDecorators.table(tables.User)
  static async updateDetails(userId, {
    name = null
  }) {
  ...
  static async updateAddress(userId, address) {
    ...
    if (!address.id) {
      ...
      await this.table.get(userId).update({
        addresses: rethinkdb.row('addresses').append(Object.assign({
          id: id
        }, address)),
        updatedAt: new Date()
      }).run();
