...
import {
  ...
  sendTemplate as sendEmailTemplate,
  EmailTemplate
} from '../../core/email';
import {
  ErrorName
...
import {
  Events as PaymentEvents,
  SwitchPayments
} from '../../core/switchPayments';
import {
  hasKeys
} from '../../core/utils';
import {
...
import {
  isReady as isCheckoutReady
} from '../checkouts/helpers';
import {
  Checkout
} from '../checkouts/models';
import {
  CheckoutSerializer
} from '../checkouts/serializers';
import {
  Order,
  OrderStatus,
  PaymentProvider
...
class OrdersHandler {
  ...
  static async get(request, reply) {
    ...
    if (!filters.userId && request.query.userId) {
      ...
      return reply(BadRequest.invalidParameters('query', {
        userId: 'Invalid'
      })).code(400);
    }
  ...
  static async post(request, reply) {
    ...
    try {
      ...
      if (!checkout) {
        ...
        return reply(BadRequest.invalidParameters('payload', {
          checkoutId: ['Invalid #1']
        })).code(400);
      } else if (checkout.archived === true) {
        log.warn({
          checkout
        }, 'Cannot create order because checkout is archived');
        return reply(BadRequest.invalidParameters('payload', {
          checkoutId: ['Archived']
        })).code(400);
        ...
        log.warn({
          checkout
        }, 'Cannot create order because checkout is not ready');
        return reply(BadRequest.invalidParameters('payload', {
          checkoutId: ['Not ready']
        })).code(400);
      }
    } catch (err) {
      if (err.name === ErrorName.PERMISSION_DENIED) {
        return reply(BadRequest.invalidParameters('payload', {
          checkoutId: ['Invalid #2']
        })).code(400);
      } else {
        log.error({
          where: 'OrdersHandler.post'
        }, 'Unable to get checkout');
        log.error(err);
...
class OrderIdHandler {
  ...
  static async get(request, reply) {
    ...
    return reply(await new OrderSerializer(request.pre.order).serialize({
      appendCheckout: true
    }));
  }
  ...
  static async patch(request, reply) {
    ...
    // Return
    return reply(await new OrderSerializer(order).serialize({
      appendCheckout: true
    }));
  }
}
...
class SwitchPaymentsWebhookHandler {
  ...
  static async post(request, reply) {
    ...
    if (!order) {
      ...
      }, '[Switch Payments] Invalid orderId');
      return reply().code(404);
    } else if (order.status !== OrderStatus.CREATED && order.status !== OrderStatus.PENDING_PAYMENT) {
      log.warn({
        orderId: request.params.orderId,
        eventId: request.query.event
      }, '[Switch Payments] Order is NOT created or pending payment');
      return reply({
    ...
    log.debug({
      orderId: request.params.orderId,
      eventId: request.query.event
    }, '[Switch Payments] Processing event');
    switchPayments.processEvent(request.query.event, {
      ...
      [PaymentEvents.CHARGE_CREATED]: async function(event) {
        log.debug({
          eventId: event.id
        }, '[Switch Payments] Charge created');
        let checkout = new Checkout(await Checkout.get(order.checkoutId));
        if (event.charge.metadata.orderId === order.id &&
          ...
          log.warn({
            event,
            orderId: order.id
          }, '[Switch Payments] Event does not match order');
        }
      },
      ...
      [PaymentEvents.INSTRUMENT_SUCCESS]: async function(event) {
        log.debug({
          eventId: event.id
        }, '[Switch Payments] Instrument success');
        if (event.charge.metadata.orderId === order.id) {

          // 1) Update order payment log
          let instrumentDetails = Object.assign({
            type: event.charge.charge_type
          }, event.instrument.redirect);
          Order.updatePaymentLog(order.id, {
            provider: PaymentProvider.SWITCH_PAYMENTS,
      ...
      [PaymentEvents.INSTRUMENT_ERROR]: async function(event) {
        log.debug({
          eventId: event.id
        }, '[Switch Payments] Instrument error');
        if (event.charge.metadata.orderId === order.id) {
          Order.updatePaymentLog(order.id, {
            provider: PaymentProvider.SWITCH_PAYMENTS,
      ...
      [PaymentEvents.PAYMENT_SUCCESS]: async function(event) {
        log.debug({
          eventId: event.id
        }, '[Switch Payments] Payment success');
        if (event.charge.metadata.orderId === order.id) {
