  ...
  static async patch(request, reply) {
    ...
    if (hasKeys(request.payload, ['name'])) {
      if (request.payload.name === '') {
        ...
        user = await User.updateDetails(userId, {
      ...
      user = await User.getByEmailAndPassword({
        email: request.auth.credentials.email,
        password: request.payload.oldPassword
      });
      if (!user) {
        return reply(BadRequest.invalidParameters('payload', {
          oldPassword: ['Invalid']
...
class AccountLoginHandlers {
  ...
  static async post(request, reply) {
    ...
    if (user.status !== UserStatus.ACTIVE) {
      log.warn({
        user
      }, 'Login attempt of disabled account');
      return reply({
...
class AccountRegisterHandlers {
  ...
  static async patch(request, reply) {
    ...
    if (!user) {
      log.error({
        decodedToken
      }, 'Unable to find user of given JWT confirmation!');
    ...
    // b) User must be pending activation
      ...
      log.warn({
        decodedToken
      }, 'Confirmation requested but user account is not pending activation');
      return reply({
        message: 'User account is not pending confirmation'
      }).code(400);
    }
    // c) Validate token
    else {
      ...
      if (!confirmationToken) {
        log.error({
          decodedToken
        }, 'Confirmation token not found in user!');
        return reply().code(500);
      } else if (confirmationToken.type !== ConfirmationToken.ACTIVATE_ACCOUNT) {
        log.warn({
          decodedToken
        }, 'Attempt to confirm account with invalid token type');
        return reply(BadRequest.invalidParameters('payload', {
          token: ['Invalid type']
        })).code(400);
        ...
        log.warn({
          decodedToken
        }, 'Account confirmation token already used');
        return reply(BadRequest.invalidParameters('payload', {
          token: ['Already used']
        })).code(400);
      }
...
class AccountResetHandlers {
  ...
  static async post(request, reply) {
    ...
    if (resetRequests.length > 0 && resetRequests[0].createdAt > new Date(new Date().getTime() - 5 * 60000)) { // 5 minutes
      return reply(BadRequest.invalidParameters('payload', {
        email: ['Please wait until making a new reset request']
      })).code(400);
    }
  ...
  static async patch(request, reply) {
    ...
    if (!user) {
      log.error({
        decodedToken
      }, 'Unable to find user of given JWT password reset!');
      return reply().code(500);
    }
    // b) User must be active
      ...
      log.warn({
        decodedToken
      }, 'Password reset requested but account is not active');
      return reply({
        message: 'Account is not active'
      }).code(400);
    }
    // c) Validate token
      ...
      if (!resetToken) {
        log.error({
          decodedToken
        }, 'Reset token not found in user!');
        return reply().code(500);
      } else if (resetToken.type !== ConfirmationToken.RESET_PASSWORD) {
        log.warn({
          decodedToken
        }, 'Attempt to reset account with invalid token type');
        return reply(BadRequest.invalidParameters('payload', {
          token: ['Invalid type']
        })).code(400);
        ...
        log.warn({
          decodedToken
        }, 'Account reset token already used');
        return reply(BadRequest.invalidParameters('payload', {
          token: ['Already used']
        })).code(400);
