  ...
  static async patch(request, reply) {
    ...
    if (hasKeys(request.payload, ['customer'])) {
      if (request.payload.customer === '') {
        ...
        return reply(BadRequest.invalidParameters('payload', {
          'customer.email': ['This field is required']
        })).code(400);
      } 
      else {
        ...
        if (Joi.string().email().validate(request.payload.customer.email).error) {
          return reply(BadRequest.invalidParameters('payload', {
            'customer.email': ['"email" must be a valid email']
          })).code(400);
        }
        ...
        if (cart.userId) {
          return reply(BadRequest.invalidParameters('payload', {
            'customer': ['This field is not allowed']
          })).code(400);
        } else {
      ...
      if (request.payload.shippingAddress === '') {
        return reply(BadRequest.invalidParameters('payload', {
          shippingAddress: ['This field is required']
        })).code(400);
      } else if (request.payload.billingAddress === '') {
        return reply(BadRequest.invalidParameters('payload', {
          billingAddress: ['This field is required']
        })).code(400);
      } else {
      ...
      if (request.payload.shippingMethod === '') {
        return reply(BadRequest.invalidParameters('payload', {
          shippingMethod: ['This field is required']
        })).code(400);
      } else if (getShippingOptions(request.pre.checkout).filter(function(option) {
          return option.value === request.payload.shippingMethod;
        }).length === 0) {
        return reply(BadRequest.invalidParameters('payload', {
