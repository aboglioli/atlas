...
class SwitchPayments {
  ...
  processEvent(eventId, handlers) {
    request.get({
      ...
      if (err) {
        ...
        }, 'Unable to fetch Switch Payments event');
      } else if (handlers.hasOwnProperty(body.type)) {
        handlers[body.type](body);
      } else {
        log.warn({
          body
        }, 'No handler provided for Switch Payment event');
      }
    });
  }
  ...
  static getInstrumentDisplay(instrumentDetails) {
    ...
    if (instrumentDetails.type === 'multibanco') {
      ...
      return [{
          label: 'Entidade',
          value: instrumentDetails.parameters.entity
        },
        {
          label: 'Referência',
          value: reference
        ...
        {
          label: 'Montante',
          value: instrumentDetails.parameters.value
        }
      ];
    } else if (instrumentDetails.type === 'mbway') {
      return [{
          label: 'Referência',
          value: instrumentDetails.parameters.reference
        },
        {
          label: 'Montante',
          value: instrumentDetails.parameters.value
