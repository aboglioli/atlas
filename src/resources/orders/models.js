...
class Order {
  ...
  static async updatePaymentLog(orderId, data) {
    return await this.table.get(orderId).update({
      paymentLog: rethinkdb.row('paymentLog').append(data)
    }).run();
  }
  ...
  static async updateStatus(orderId, status, description, details) {
    ...
    // Validate status
    if (Object.keys(OrderStatus).map(function(key) {
